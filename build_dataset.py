...
def main():
    ...
    for dataset_type, labels in LABEL_GROUPS.items():
        csv_file = (meta_root / f"{dataset_type}_samples.csv").open("w", newline="", encoding="utf-8")
    ...
    pfs_file = (meta_root / "pfs_samples.csv").open("w", newline="", encoding="utf-8")
    ...
    for record in valid_reports:
        ...
        for dataset_type in ("pftg", "pfus", "xray"):
            for img_rel in copy_images(report_dir / dataset_type, img_root / dataset_type, uid_prefix):
                row = {
        ...
        for img_rel in copy_images(report_dir / "pfs", img_root / "pfs", uid_prefix):
    ...
    # Close all open files and write label statistics
